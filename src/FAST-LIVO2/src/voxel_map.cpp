...
void loadVoxelConfig(rclcpp::Node::SharedPtr &node,
  ...
  node->declare_parameter<vector<int64_t>>("lio.layer_init_num",
...
void VoxelMapManager::BuildVoxelMap() {
  ...
  for (uint i = 0; i < plsize; i++) {
    ...
    auto iter = voxel_map_.find(position);
    if (iter != voxel_map_.end()) {
      VoxelOctoTree *voxel = iter->second->second;
      voxel->temp_points_.push_back(p_v);
      voxel->new_points_++;
      ...
      auto cache_it = voxel_map_cache_.begin();
      voxel_map_[position] = cache_it;
      std::cout << "Building Voxel Map..." << endl;
      cache_it->second->quater_length_ = voxel_size / 4;
      cache_it->second->voxel_center_[0] = (0.5 + position.x) * voxel_size;
      cache_it->second->voxel_center_[1] = (0.5 + position.y) * voxel_size;
      cache_it->second->voxel_center_[2] = (0.5 + position.z) * voxel_size;
      cache_it->second->temp_points_.push_back(p_v);
      cache_it->second->new_points_++;
      cache_it->second->layer_init_num_ = layer_init_num;
    }
  }
  for (auto iter = voxel_map_.begin(); iter != voxel_map_.end(); ++iter) {
    iter->second->second->init_octo_tree();
  }
  std::cout << "Builded Voxel Map..." << endl;
}
...
void VoxelMapManager::UpdateVoxelMap(
  ...
  for (uint i = 0; i < plsize; i++) {
    ...
    auto iter = voxel_map_.find(position);
    // if (iter != voxel_map_.end()) {
    // voxel_map_[position]->UpdateOctoTree(p_v); } else
    // {
    //   VoxelOctoTree *octo_tree = new VoxelOctoTree(max_layer, 0,
    //   layer_init_num[0], max_points_num, planer_threshold);
    //   voxel_map_[position] = octo_tree;
    //   voxel_map_[position]->layer_init_num_ = layer_init_num;
    //   voxel_map_[position]->quater_length_ = voxel_size / 4;
    //   voxel_map_[position]->voxel_center_[0] = (0.5 + position.x) *
...
void VoxelMapManager::clearMemOutOfMap(const int &x_max, const int &x_min,
  ...
  for (auto it = voxel_map_.begin(); it != voxel_map_.end();) {
    ...
    if (should_remove) {
      ...
      voxel_map_cache_.erase(cache_it);
