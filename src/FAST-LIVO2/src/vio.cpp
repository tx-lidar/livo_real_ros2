...
//           // cv::circle(img_cp, cv::Point2f(pc[0], pc[1]), 3, cv::Scalar(0,
//           255,
//           // 255), -1, 8);
//           voxel_in_fov = true;
...
//         // if (abs(cos_theta) < 0.08) continue; // 0.5 60 degree 0.34 70
//         degree
...
//             double normal_update = (pt->normal_ -
//             pt->previous_normal_).norm();
...
//           float other_sum =
//               std::accumulate(patch_cache, patch_cache + patch_size_total,
//               0.0);
...
//           NCC_down2 +=
//               (patch_cache[ind] - other_mean) * (patch_cache[ind] -
//               other_mean);
//         }
...
//                   cv::FONT_HERSHEY_COMPLEX, 0.4, cv::Scalar(0, 0, 255), 1, 8,
//                   0);
//       cv::putText(ref_cur_combine_temp, cur_optimize,
...
//                     cv::Scalar(255, 255, 255), 1);
//       cv::circle(ref_cur_combine_temp,
//                  cv::Point2f(ref_ftr->px_[0] + img_cur.cols,
//                  ref_ftr->px_[1]), 1, cv::Scalar(0, 0, 255), -1, 8);
//       cv::circle(ref_cur_combine_temp, cv::Point2f(pc[0], pc[1]), 1,
//                  cv::Scalar(0, 255, 0), -1, 8);
//       cv::circle(ref_cur_combine_temp, cv::Point2f(pc_prior[0], pc_prior[1]),
//       1,
//                  cv::Scalar(255, 255, 255), -1, 8);
//       cv::imwrite(dir + std::to_string(new_frame_->id_) + "_" +
//                       std::to_string(ref_ftr->id_) + "_" +
//                       std::to_string(num) +
//                       ".png",
//                   ref_cur_combine_temp);
...
//             const Vector2f px_ref(px_patch + ref_ftr->px_.cast<float>());
//             uint8_t pixel_value =
//                 (uint8_t)vk::interpolateMat_8u(img_ref, px_ref[0],
//                 px_ref[1]);
//             it_normal[width * j + i] = pixel_value;
//           }
//         }
...
//   cv::cvtColor(ref_cur_combine_normal, ref_cur_combine_normal, CV_GRAY2BGR);
//   cv::absdiff(img_photometric_error, result_normal, img_photometric_error);
//   cv::hconcat(img_photometric_error, new_frame_->img_,
//   ref_cur_combine_error);
...
//                   ".png",
//               ref_cur_combine_error);
//   cv::imwrite(dir + std::to_string(new_frame_->id_) + "_0_" + "normal" +
//   ".png",
//               ref_cur_combine_normal);
// }
...
void VIOManager::processFrame(
  ...
  retrieveFromVisualSparseMap(img, pg, vm_map);
  ...
  if (plot_flag)
    projectPatchFromRefToCur(vm_map);
  // projectPatchFromRefToCur(vm_map);
...
// void VIOManager::processFrame(
//     cv::Mat &img, vector<pointWithVar> &pg,
//     const unordered_map<VOXEL_LOCATION, VoxelOctoTree *> &vm_map,
//     double img_time) {
//   if (width != img.cols || height != img.rows) {
//     if (img.empty())
//       printf("[ VIO ] Empty Image!\n");
