...
void LIVMapper::readParameters(rclcpp::Node::SharedPtr &node) {
  // declare parameters
  this->node->declare_parameter<std::string>("common.lid_topic",
                                             "/livox/lidar");
  ...
  this->node->declare_parameter<bool>("preprocess.feature_extract_enabled",
                                      false);
  ...
  this->node->declare_parameter<vector<double>>("extrin_calib.extrinsic_T",
                                                vector<double>{});
  this->node->declare_parameter<vector<double>>("extrin_calib.extrinsic_R",
                                                vector<double>{});
  this->node->declare_parameter<vector<double>>("extrin_calib.Pcl",
                                                vector<double>{});
  this->node->declare_parameter<vector<double>>("extrin_calib.Rcl",
                                                vector<double>{});
...
void LIVMapper::initializeComponents(rclcpp::Node::SharedPtr &node) {
  ...
  if (!vk::camera_loader::loadFromRosNs(this->node, "parameter_blackboard",
                                        vio_manager->cam))
...
void LIVMapper::livox_pcl_cbk(
    livox_ros_driver2::msg::CustomMsg::ConstSharedPtr msg_in) {
  if (!lidar_en)
    return;
  mtx_buffer.lock();
...
cv::Mat
LIVMapper::getImageFromMsg(sensor_msgs::msg::Image::ConstSharedPtr img_msg) {
  cv::Mat img;
...
// static int i = 0;
void LIVMapper::img_cbk(sensor_msgs::msg::Image::ConstSharedPtr msg_in) {
  if (!img_en)
    return;
